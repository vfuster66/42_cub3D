...
/*   Updated: 2023/12/10 20:42:04 by parallels        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int validate_texture_path(const char *path)
{
    ...
    if (extension == NULL)
    {
        printf("Chemin de texture invalide : pas d'extension\n"); // Ajoutez ce printf
        return (0);
    }
    if (strcmp(extension, ".xpm") != 0)
    {
        printf("Chemin de texture invalide : extension incorrecte\n"); // Ajoutez ce printf
        return (0);
    }
    printf("Chemin de texture valide : %s\n", path); // Ajoutez ce printf
    return (1);
}
...
int handle_texture_path(char *key, char *value, t_map *map_info)
{
    ...
    if (texture_path != NULL && validate_texture_path(value))
    {
        *texture_path = strdup(value);
        ...
        printf("Chemin de la texture %s trouvé : %s\n", key, *texture_path); // Ajoutez ce printf
        return (1);
    }
    return (0);
}
