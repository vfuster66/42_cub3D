...
int	fill_color_textures(t_data *data, t_texture *textures, char *line, int j)
{
	if (line[j + 1] && ft_isprint(line[j + 1]))
		return (error_msg(data->mapinfo.path, ERROR_FLOOR_CEILING, ERROR));
	if (!textures->ceiling && line[j] == 'C')
	{
		textures->ceiling = set_rgb_colors(line + j + 1);
		if (textures->ceiling == 0)
			return (error_msg(data->mapinfo.path, ERROR_COLOR_CEILING, ERROR));
	}
	else if (!textures->floor && line[j] == 'F')
	{
		textures->floor = set_rgb_colors(line + j + 1);
		if (textures->floor == 0)
			return (error_msg(data->mapinfo.path, ERROR_COLOR_FLOOR, ERROR));
	}
	else
		return (ERROR_FILE_is_directory(data->mapinfo.path, ERROR_FLOOR_CEILING, ERROR));
	return (0);
}