...
int init_mlx_window(t_mlx *mlx_info, int width, int height, char *title)
{
    ...
    mlx_info->win_width = width;
    mlx_info->win_height = height;
    mlx_info->win_ptr = mlx_new_window(mlx_info->mlx_ptr, mlx_info->win_width, mlx_info->win_height, title);
    if (!mlx_info->win_ptr)
	{
...
int	main(int ac, char **av)
{
	...
	if (check_arguments(ac, av) != 0)
		return (1);
	memset(&map_info, 0, sizeof(t_map));
    if (process_map_file(av[1], &map_info) != 0)
        return (1);
    if (init_mlx_window(&mlx_info, 640, 480, "cub3D") != 0) {
        return (1);
    }
